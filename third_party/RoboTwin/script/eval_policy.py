...
def _safe_robot_call(name, fn):
    try:
        return fn()
    except Exception as exc:
        return {"error": f"{name}: {repr(exc)}"}


def _collect_episode_end_state(task_env, *, episode_idx, seed, success, video_path):
    robot = getattr(task_env, "robot", None)
    record = {
        "episode_idx": int(episode_idx),
        "seed": int(seed),
        "success": bool(success),
        "instruction": _safe_robot_call("instruction", task_env.get_instruction),
    ...
    if robot is None:
        record["robot_error"] = "task_env.robot is missing"
        return record

    record["left_drive_qpos"] = _safe_robot_call("left_drive_qpos", robot.get_left_arm_jointState)
    record["right_drive_qpos"] = _safe_robot_call("right_drive_qpos", robot.get_right_arm_jointState)
    record["left_real_qpos"] = _safe_robot_call("left_real_qpos", robot.get_left_arm_real_jointState)
    record["right_real_qpos"] = _safe_robot_call("right_real_qpos", robot.get_right_arm_real_jointState)
    record["left_ee_pose"] = _safe_robot_call("left_ee_pose", robot.get_left_ee_pose)
    record["right_ee_pose"] = _safe_robot_call("right_ee_pose", robot.get_right_ee_pose)
    record["left_tcp_pose"] = _safe_robot_call("left_tcp_pose", robot.get_left_tcp_pose)
    record["right_tcp_pose"] = _safe_robot_call("right_tcp_pose", robot.get_right_tcp_pose)
    return _json_safe(record)


def _append_episode_end_state(path, record):
...
def main(usr_args):
    ...
    args["eval_episode_state_path"] = str(save_dir / "episode_end_states.jsonl")
...
def eval_policy(task_name,
    ...
    while succ_seed < test_num:
        ...
        if TASK_ENV.eval_video_path is not None:
            ...
            current_video_path = renamed_video_path

        end_state_record = _collect_episode_end_state(
            TASK_ENV,
            episode_idx=now_id,
            seed=now_seed,
            success=succ,
        ...
        _append_episode_end_state(args.get("eval_episode_state_path"), end_state_record)
        if hasattr(model, "print_timing_rollout"):
            model.print_timing_rollout(
                episode_idx=now_id,
                seed=now_seed,
                success=succ,
            )
